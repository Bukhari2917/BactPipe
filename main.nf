...
process FASTQC {
    ...
    script: "mkdir -p fastqc_results && fastqc ${r1} ${r2} -o fastqc_results/ -t ${task.cpus}"
}
...
process TRIM {
    ...
    input: tuple val(sample_id), path(r1), path(r2)
    output: tuple val(sample_id), path("${sample_id}_R1.fastq"), path("${sample_id}_R2.fastq")
    script: "fastp -i ${r1} -I ${r2} -o ${sample_id}_R1.fastq -O ${sample_id}_R2.fastq -q 20 -l 50 --thread ${task.cpus}"
}
...
process ASSEMBLE {
    ...
    input: tuple val(sample_id), path(r1), path(r2)
    output: tuple val(sample_id), path("contigs.fasta")
    script: "spades.py -1 ${r1} -2 ${r2} -o spades_out --isolate --threads ${task.cpus} && cp spades_out/contigs.fasta ."
}

process QUAST {
    tag "${sample_id}"
    publishDir "${params.outdir}/stats", mode: 'copy'
    input: tuple val(sample_id), path(assembly)
    output: path "quast_results"
    script: "quast.py ${assembly} -o quast_results --threads ${task.cpus}"
}
...
process BUSCO {
    ...
    script: "busco -i ${assembly} -o busco_results -l bacteria_odb10 -m genome --cpu ${task.cpus}"
}
...
process PRODIGAL {
    ...
    input: tuple val(sample_id), path(assembly)
    output: tuple val(sample_id), path("proteins.faa")
...
process ABRICATE_AMR {
    ...
    script: "abricate --db card ${assembly} > amr_card.tsv 2>/dev/null || echo 'No AMR genes found' > amr_card.tsv"
}
...
process ABRICATE_VIR {
    ...
    script: "abricate --db vfdb ${assembly} > virulence.tsv 2>/dev/null || echo 'No virulence genes found' > virulence.tsv"
}
...
process CRISPR {
    ...
    script: "echo 'CRISPR analysis - minced not available' > crispr.txt"
}
...
process MLST {
    ...
    script: "mlst ${assembly} > mlst.txt 2>/dev/null || echo 'No MLST scheme found' > mlst.txt"
}
...
process EGGNOG {
    ...
    input: path proteins
    output: path "kegg_pathways.txt", path "cog_categories.txt", path "go_terms.txt"
    script: """
...
process MULTIQC {
    ...
    input: path fastqc, path trim, path assembly
    output: path "multiqc_report.html"
    script: "multiqc . --filename multiqc_report.html --force"
}
